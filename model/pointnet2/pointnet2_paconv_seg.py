...
from model.transformer import transfomer as TF
...
class PointNet2INSSeg(nn.Module):
    r"""
          PointNet2 with single-scale grouping
          Semantic segmentation network that uses feature propogation layers
          Parameters
          ...
          use_xyz: bool = True
              Whether or not to use the xyz position of a point as a feature
      """
    def __init__(self, c=3, use_xyz=True, args=None, hidden_dim=args['hidden_dim']):
        super().__init__()
        self.nsamples = args.get('nsamples', [32, 32, 32, 32])
        self.npoints = args.get('npoints', [None, None, None, None])
        ...
        self.fp_mlps = args.get('fp_mlps', [[128 + c, 128, 128, 128], [256 + 64, 256, 128], [256 + 128, 256, 256],
                                            [512 + 256, 256, 256]])
        self.paconv = args.get('pointnet2_paconv', [True, True, True, True, False, False, False, False])
        self.num_embed = args.get('num_embed', 5)
        ...
        self.FP_modules.append(PointNet2FPModule(mlp=self.fp_mlps[2], use_paconv=self.paconv[6], args=args))
        self.FP_modules.append(PointNet2FPModule(mlp=self.fp_mlps[3], use_paconv=self.paconv[7], args=args))
        self.hiddem_dim = block.Conv2d(131, hidden_dim, bn=True)
        self.transformer = TF.build_transformer(args)
        self.class_embed = nn.Linear(hidden_dim, self.num_embed)
    ...
    def forward(self, pointcloud: torch.cuda.FloatTensor):
        ...
        ins_input = torch.cat((l_features[0].unsqueeze(-1),xyz.transpose(1, 2).unsqueeze(-1)),dim=1)
        ins_input = self.hiddem_dim(ins_input)
        ins_output = self.transformer(ins_input)
        ins_output = self.class_embed(ins_output)
        #
        return ins_output.transpose(1,2)
...
if __name__ == "__main__":
    ...
    model = PointNet2INSSeg(args=args).cuda()
    ...
    # model = PointNet2SSGSeg(c=C, k=K, use_xyz=False, args=dict()).cuda()
    # optimizer = optim.SGD(model.parameters(), lr=5e-2, momentum=0.9, weight_decay=1e-4)
